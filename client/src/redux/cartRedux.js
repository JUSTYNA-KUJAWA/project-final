...
export const getCartTotal = ({cart}) => cart.reduce((acc, current) => {
...
export const reducer = (statePart = initialState, action = {}) => {
  switch (action.type) {
    case ADD_TO_CART:
      ...
      const orderProduct = statePart.find(product => product._id === action.payload._id);
      if(orderProduct) {
        orderProduct.amount = orderProduct.amount + action.payload.amount;
        orderProduct.totalPrice += action.payload.totalPrice;
        return statePart;
      } else {
